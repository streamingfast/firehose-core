...
- `--reader-node-line-buffer-size` is now the normal size of the line buffer, and its default goes from 200 MiB to 100 MiB. The buffer grows past it when a longer line comes in, up to `2861913428` bytes (about 2.67 GiB, the longest line whose block fits in a 2 GiB response, a line over that fails the reader), and shrinks back to it once 100 blocks in a row used less than half of it. Reading the longest line takes up to 5.4 GiB of memory, and a grown buffer stays allocated until it shrinks back.
