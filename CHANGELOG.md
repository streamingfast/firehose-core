...
- `--reader-node-line-buffer-size` is now the maximum size the line buffer grows to, and its default goes from 200 MiB to about 2.67 GiB (`2861913428`), which is also the highest accepted value: a longer line decodes to a block that does not fit in a 2 GiB response. The buffer starts at 100 MiB, grows when a longer line comes in and shrinks back once 100 blocks in a row used less than half of it. Reading the longest line takes up to 5.4 GiB of memory, and a grown buffer stays allocated until it shrinks back.
