...
- Added `firecore tools substreams purge <store-url>` which deletes substreams module caches that have not been used recently, reading the `last_used*.zst` markers that `substreams-tier1` refreshes on every request it serves. No database involved, the bucket is the only source of truth.

  Retention is per billing plan (`--retention default=30d,pro=30d,scaling=14d,free=3d`), a module folder being kept as soon as one of its markers is within its own plan's retention, so a folder last used 15 days ago by a `pro` request survives a 3 days `free` retention. Plans with no explicit retention fall back to the `default` one, and a folder carrying no marker at all is never touched unless `--purge-without-last-used` is set.
  ...
  Storage goes entirely through `dstore`, so every store it supports works (`gs://`, `s3://`, `az://`, `file://`, a bare local path), with no backend-specific code: names, sizes and modification times all come out of the listings it was already making. A folder listing is paged one round trip at a time however few folders come back, so on the stores that bound a listing service-side (`gs://`, `s3://`) each level is cut into 17 slices listed concurrently, which takes the module-hash level of a network holding 39k folders from 8.2s to 1.6s.

  Networks are processed one at a time and fully isolated: a network that cannot be listed, or whose files refuse to be deleted, is reported and the run moves on to the next one, the command exiting non-zero with a per-network summary. Use `--dry-run` (`-n`) to see what would go, `--force` (`-f`) to skip the confirmation prompt, `--network` to restrict to some networks (default is every network under the root), and `--daemon --interval 12h` to keep purging unattended (a failed pass is retried at the next interval instead of killing the daemon).
