...
- Added `firecore tools substreams purge <state-url>` which deletes substreams module caches that have not been used recently, reading the `last_used*.zst` markers that `substreams-tier1` refreshes on every request it serves. The required state URL matches direct tier1 layouts (`<state-url>/<hash>` and `<state-url>/<tag>/<hash>`) and shared network roots (`<state-url>/<network>/substreams-states/<tag>/<hash>`); `--network` is optional for restricting shared network roots. No database involved, the store is the only source of truth.
  ...
  Retention can be per billing plan (`--retention default=30d,pro=30d,scaling=14d,free=3d`), a module folder being kept as soon as one of its markers is within its own plan's retention, so a folder last used 15 days ago by a `pro` request survives a 3 days `free` retention. Plans with no explicit retention fall back to the `default` one, and a folder carrying no marker at all is never touched unless `--purge-without-last-used` is set.

  A condemned folder is emptied completely, `substreams.spkg.zst` and `substreams.partial.spkg.zst` included, unless `--keep` names glob patterns to spare. The scan itself downloads nothing: `substreams-tier1` overwrites a marker on every request it serves, so the object's last-write time is the usage date and the listing already carries it (`--read-marker-contents` downloads each marker and reads the date tier1 stored inside it instead, which is what you want on any backend once the objects have been copied, migrated or rsynced, since that resets the last-write time and would otherwise make every module look freshly used). Finding the folders costs one small request per module folder, all independent, so it scales with `--scan-workers` (256 by default, worth raising on a large network); the state and output files of a folder are only enumerated once it is condemned. `--scan-only` reports what is past retention without listing or deleting anything.

  Storage goes entirely through `dstore`, so every store it supports works (`gs://`, `s3://`, `az://`, `file://`, a bare local path), with no backend-specific code: names, sizes and modification times all come out of the listings it was already making. The command accepts direct tier1 paths (`[tag/]module-hash`) and shared roots (`network/substreams-states/[tag/]module-hash`). A folder listing is paged one round trip at a time however few folders come back, so on the stores that bound a listing service-side (`gs://`, `s3://`) each level is cut into 17 slices listed concurrently, which takes the module-hash level of a network holding 39k folders from 8.2s to 1.6s.
...
- Added `firecore tools substreams prune-states <store-url> --keep-every <blocks> --truncate-below-block <block>` to delete intermediate store snapshots (`<end>-<start>.kv` files) and reclaim disk space on long-lived Substreams. The first snapshot of each `--keep-every`-aligned window is kept, windows starting on multiples of `--keep-every` whatever the module's initial block is (the first window is simply shorter). Only snapshots whose end block is at or below `--truncate-below-block` are thinned — everything above stays untouched, so requests in flight and the live head keep their nearby resume points — and the most recent snapshot of each module is always kept. `--minimum-age`, if set, additionally spares any snapshot last modified more recently than that (the modification time comes straight out of the listing). `substreams-tier1` rebuilds stores from the last remaining snapshot before the requested block, so pruning trades disk space for reprocessing time. Deletion asks for confirmation unless `--force` is given; supports `--dry-run`. Module folders are discovered the same way `purge` does — direct tier1 layouts, cache tags and shared `network/substreams-states/` roots — and only each module's `states/` prefix is then listed; a URL pointing inside a single module folder falls back to walking the whole tree.
...
- New `--substreams-tier1-cpu-eviction-*` flags letting `substreams-tier1` shed requests when its own cgroup reports the CPU saturated. `--substreams-tier1-cpu-eviction-mode` (default `off`, so nothing changes until you set it) selects what it does: `observe` logs the requests it would cancel without cancelling any, `dev-only` cancels development-mode requests, `full` cancels production ones as well. See the `substreams` bump below for what an eviction round does.

  The rest are tunables, each defaulting to the value the eviction was designed around: `--substreams-tier1-cpu-eviction-threshold` (`0.90`) and `--substreams-tier1-cpu-eviction-sustain` (`15s`) say when the instance counts as overloaded, `--substreams-tier1-cpu-eviction-recover-threshold` (`0.75`) and `--substreams-tier1-cpu-eviction-recover-sustain` (`30s`) when it stops being, `--substreams-tier1-cpu-eviction-target-ratio` (`0.75`) how far down a round of eviction cuts, `--substreams-tier1-cpu-eviction-interval` (`5s`) and `--substreams-tier1-cpu-eviction-cooldown` (`15s`) how often it looks and how often it cuts, `--substreams-tier1-cpu-eviction-drain-delay` (`8s`) how long it waits after going unready before cancelling anything, and `--substreams-tier1-cpu-eviction-min-age` (`90s`) and `--substreams-tier1-cpu-eviction-min-burn-cores` (`0.05`) which requests are never candidates. `--substreams-tier1-cpu-eviction-nominal-capacity` (`0`, meaning `--substreams-tier1-active-requests-soft-limit`) scales the new `substreams_tier1_effective_active_requests` metric only.

  Thresholds and ratios are fractions of the CPU quota set on the cgroup. An instance with no CPU quota, or running where the cgroup v2 CPU files cannot be read, logs a warning at startup and keeps running with the eviction disabled.

- Bumped `substreams` to [v1.22.1-0.20260902140046-aa86dfa0e34a](https://github.com/streamingfast/substreams/compare/1cffa6c10a8d...aa86dfa0e34a):
