...
- Bumped `substreams` to `feature/substreams-squasher-grpc-plugin` so the squasher plugin DSN is available. `grpc://` / `grpcs://` registration comes from public `github.com/streamingfast/substreams/squash/grpc` (no private `substreams-squasher` module). That pin is behind `develop`: `--substreams-tier1-cpu-eviction-*` flags are still accepted so existing configs start, but they are no longer wired (CPU eviction is not on this substreams branch).
