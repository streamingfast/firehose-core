...
- `firecore tools check merged-blocks` no longer reports a range as healthy when a merged-blocks file holds blocks that do not belong to it. It only listed the store and looked for holes in the sequence of file names, so a bundle such as `0096336100` holding block `96336220` passed with "🆗 No hole found" while every Firehose and Substreams read over that range failed with `merged blocks file "0096336100" contains block 96336220, beyond the configured bundle size of 100 blocks`. The new `--validate-blocks` (`-b`) flag reads each file and checks every block against the bundle boundaries claimed by its file name, the same rule `bstream`'s `FileSource` enforces at serving time, and reports the offending files in the summary. `--print-stats` and `--print-full` imply it. Without the flag the check stays a file-name listing, and the summary now says so instead of implying the content was verified.

  Blocks *below* the base num are not reported: the merger deliberately carries the last block of the previous bundle into each file so the hub can join on one-block files, and readers skip them.
...
- Bumped `substreams` to [v1.21.1-0.20260819164836-8ccf4c0e07d7](https://github.com/streamingfast/substreams/compare/v1.21.0...8ccf4c0e07d7):
