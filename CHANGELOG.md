...
- The merger no longer fails (or crashes) at startup when the most recent merged-blocks bundles hold no block. On chains that skip block numbers a whole bundle range can contain nothing, and the merger writes an empty merged-blocks file so boundaries stay contiguous. Reading the last block back reported `unable to read file header: EOF` on the zero-byte files older mergers wrote, and nil-dereferenced on the header-only files current ones write. Startup now walks back to the last bundle that actually has a block.
