...
func TestScanOutputFolders(t *testing.T) {
	...
	const noPrune = "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
	...
	old := []string{
		...
		"eth-mainnet/substreams-states/" + noPrune + "/outputs/0000001000-0000002000.output.zst",
		"eth-mainnet/substreams-states/" + noPrune + "/DO_NOT_PRUNE",
	}
	fresh := []string{
	...
	backend, err := newPurgeStore(ctx, base+"/eth-mainnet", 4, zap.NewNop())
	require.NoError(t, err)
	defer backend.Close()

	folders, skipped, err := backend.DiscoverModuleFolders(ctx)
	require.NoError(t, err)
	...
	// The output module (spkg present) and the DO_NOT_PRUNE-protected one are kept
	// untouched; elsewhere only files both fully below the block and older than the age
	// go, whatever the cache tag.
	modules, outputModulesKept, protected, scanErrors := scanOutputFolders(ctx, backend, cfg, folders, zap.NewNop())
	...
	assert.Equal(t, "substreams-states/"+plain+"/outputs/0000001000-0000002000.output.zst", modules[0].files[0].name)
	...
	// but never the DO_NOT_PRUNE-protected one.
	...
	modules, outputModulesKept, _, _ = scanOutputFolders(ctx, backend, cfg, folders, zap.NewNop())
	assert.Zero(t, outputModulesKept)
	require.Len(t, modules, 2)

	// The URL may point at any level: substreams-states folder, a cache tag, or one module.
	for _, level := range []struct {
		url  string
		want int
	}{
		{base + "/eth-mainnet/substreams-states", 4},
		{base + "/eth-mainnet/substreams-states/mmap-stores", 1},
		{base + "/eth-mainnet/substreams-states/" + plain, 1},
		{base + "/eth-mainnet/substreams-states/mmap-stores/" + tagged, 1},
	} {
		levelBackend, err := newPurgeStore(ctx, level.url, 4, zap.NewNop())
		require.NoError(t, err)
		levelFolders, skipped, err := levelBackend.DiscoverModuleFolders(ctx)
		require.NoError(t, err, level.url)
		require.Zero(t, skipped)
		assert.Len(t, levelFolders, level.want, level.url)
	}
}
