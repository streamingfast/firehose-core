...
func RegisterMergerApp(rootLog *zap.Logger) {
	launcher.RegisterApp(rootLog, &launcher.AppDef{
		...
		RegisterFlags: func(cmd *cobra.Command) error {
			...
			cmd.Flags().Uint64("merger-prune-one-block-files-after", 100, "Number of blocks below the last merged block after which we delete merged one-block-files. Increase so a relayer/firehose that fell behind can still bridge the gap and relink (clamped to the merged-blocks bundle size minimum)")
		...
		FactoryFunc: func(runtime *launcher.Runtime) (launcher.App, error) {
			...
			return merger.New(&merger.Config{
				...
				BundleSize:                   viper.GetUint64("common-merged-blocks-bundle-size"),
