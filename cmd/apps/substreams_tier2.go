...
func RegisterSubstreamsTier2App[B firecore.Block](chain *firecore.Chain[B], rootLog *zap.Logger) {
	...
	launcher.RegisterApp(rootLog, &launcher.AppDef{
		...
		RegisterFlags: func(cmd *cobra.Command) error {
			...
			cmd.Flags().String("substreams-tier2-hosted-store-registry-address", "", "Deprecated: no longer used, tier1 resolves hosted foundational stores and sends the concrete endpoints to tier2")
			cmd.Flags().MarkDeprecated("substreams-tier2-hosted-store-registry-address", "tier1 now resolves hosted foundational stores and sends the concrete endpoints to tier2, this flag is ignored")
			// all substreams
