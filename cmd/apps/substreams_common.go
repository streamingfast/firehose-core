...
func registerCommonSubstreamsFlags(cmd *cobra.Command) {
	registerSSOnce.Do(func() {
		...
		cmd.Flags().String("substreams-stores-scratch-space", "", "Local directory used as scratch space for store KV backends (e.g. mmap files); defaults to OS temp dir if unset")
		cmd.Flags().String("substreams-stores-backend", "mmap", "KV backend to use for FullKV stores: 'mmap' (default, bbolt-backed) or 'memory'")
	})
}
