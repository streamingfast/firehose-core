...
func RegisterSubstreamsTier1App[B firecore.Block](chain *firecore.Chain[B], rootLog *zap.Logger) {
	...
	launcher.RegisterApp(rootLog, &launcher.AppDef{
		...
		RegisterFlags: func(cmd *cobra.Command) error {
			...
			cmd.Flags().Uint64("substreams-tier1-output-bundle-size", 1, "[EXPERIMENTAL] size of 'bundles' of BlockScopedData (requires client-side specific handling)")
			// all substreams
		...
		FactoryFunc: func(runtime *launcher.Runtime) (launcher.App, error) {
			...
			config.OutputBundleSize = viper.GetUint64("substreams-tier1-output-bundle-size")
